// ListCtrlCL.cpp : Defines the initialization routines for the DLL.
//
...
//	ON_NOTIFY_REFLECT(NM_RCLICK, &CListCtrlCl::OnNMRClick)
END_MESSAGE_MAP()
// CListCtrlCl 消息处理程序
//在此源代码有点问题
...
bool CListCtrlCl::FindItemColor(int col, int row, COLORREF &color) //查找颜色
{
	...
	for (POSITION pos = m_ptrListItem.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pColor->nCol == col && pColor->nRow == row)
		{
			flag = 1;
			color = pColor->rgb;
			//break;//这里去掉，因为先后设置了多个颜色，他这里是查找到第一个就立马返回了。去掉这个break后就是以最后一个为准
		}
	}
	if (1 == flag)
	{
